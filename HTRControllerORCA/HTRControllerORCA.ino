...
bool humanReadable = false; // generate human readable updates
...
//  if (Serial.available()<2) return; // if there's not at least two bytes available there's no valid incoming command
  ...
  switch (Serial.read()) // read the first byte of the incomming data
  {
    ...
    break;
    case 'O' : // stop human readable updates
      humanReadable = false; 
    break;    
    case '?' : // display settings
      humanReadable = true; // must be someone accessing device through a terminal program, start producing human readable output
      ...
      Serial.println(F("O - Orca mode, stop generating human readable updates"));
...
void loop() 
{
  ...
  pressure = float(iPressureADCValue) * slopeADCtoPSIG + offsetADCtoPSIG; // 10 bit ADC with 5V full scale, 0.5 V is 0 psig, 4.5 V is 30 psig
